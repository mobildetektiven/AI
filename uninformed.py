...
# domainindependet uniform cost search algorithm 
# Each time a cask is either loaded or unloaded state.mission_num is increased, this is used
# to avoid repeated states, and to avoid uneccesary list operations 
class SearchProblem:
	...
	def getStateChildren(self,state):
		...
		if node_has_stack != None:
			...
			if stack_at_current_node.stackSpaceOccupied() > 0 and not state.loaded:
				...
				#update heuristic cost based on blocking of goal cask or if cask is goal cask
				...
				temp_state = State(state.location,True,cost_of_action,new_total_cost,state,"load", cask, stacks_copy,cask_list, {}, state.mission_num + 1)
			...
			#If CTS has cask unload if available space in stack
				...
				if stack_at_current_node.stackSpaceFree() >= state.cask.length:
					...
					new_total_cost = (state.total_cost + cost_of_action)
					#update heuristic cost based on blocking of goal cask or if cask i goal cask

					stack_at_current_node.addCaskToStack(state.cask)
					stacks_copy[state.location] = stack_at_current_node
					temp_state = classes.State(state.location,False,cost_of_action,new_total_cost,state,"unload", None, stacks_copy, state.cask_list,{}, state.mission_num + 1)
					#temp_state.mission_num.mission_num += 1
					if not self.stateExplored(temp_state):
						children.append(temp_state)
		...
		for col_num in range(0,self.num_nodes):
			if self.adj_matrix[node_num][col_num] < inf:
				...
				temp_state = State(location,loaded,cost_of_action, new_total_cost, state,"move", state.cask, state.stacks,state.cask_list,state.casks_handled, state.mission_num)
				if not self.stateExplored(temp_state):
					children.append(temp_state)
		return children
